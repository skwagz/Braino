...
These rules define how an AI coding agent — Codex, Claude Code, Cursor, or any other assistant working in this repo — should plan, execute, verify, communicate, and recover when working in a real codebase. They apply regardless of which tool is at the keyboard or who is driving it. Optimize for correctness, minimalism, and developer experience.
...
### 2. Subagent / Multi-Agent Strategy (Parallelize Intelligently)

- Where the tool supports it, use subagents or sub-tasks to keep the main context clean and to parallelize:
...
- **Use a standard naming scheme** for tasks:
  - `YYYY-MM-DD short_name` (or repo convention) to keep history searchable
...
- The full commit convention (types, message format, process) lives in `AGENTS.md` — it is the single source of truth regardless of which tool is committing.
...
1. Identify entry points:
   - `AGENTS.md`, `README`, `CONTRIBUTING`, `Makefile`, `package.json`, build scripts, service manifests.
...
## Data Migration and Schema Change Guidelines
...
## Concurrency and Consistency Guardrails
...
## AI-Specific Hallucination Mitigation
